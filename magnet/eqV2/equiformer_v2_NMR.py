...
class EquiformerV2_NMR(BaseModel):
    ...
    def __init__(
        ...
        # predicting scalars (e.g., NMR shieldings) for each node
    ...
    def forward(self, data):
        ...
        # filter out certain edges here
        # `data.keys` is a method in torch_geometric >= 2.6, so test the attribute directly
        # (version-independent); this branch only runs for the solvent-aware MagNET-x models.
        if (self.filter_solvent_edges) and hasattr(data, "solute"):
            contains_solvent_edge = data.solute[edge_index] == 0

            #is_solvent_solvent_edge = contains_solvent_edge[0,:] & contains_solvent_edge[1,:] # solvent-solvent edge
            #remove_solvent_edge = (edge_distance > 4.0) & is_solvent_solvent_edge
                
            is_solvent_edge = contains_solvent_edge[1,:] # edges incoming to a solvent atom (target atom)
        ...
        if self.share_atom_edge_embedding and self.use_atom_edge_embedding:
            ...
            # (if self.share_atom_edge_embedding == False, apply this in transformer_block.py instead.)
